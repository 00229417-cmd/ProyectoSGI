...
def login_page():
    # Layout: formulario de login y justo debajo el registro compacto
    col1, col2 = st.columns([2, 1])
    with col1:
        _render_login_form()
        # aquí colocamos el registro justo debajo del login (compacto)
        st.markdown("---")
        _render_register_compact()

    with col2:
        st.markdown("#### Información")
        st.write("Acceso limitado. Solo administradores pueden crear roles.")
