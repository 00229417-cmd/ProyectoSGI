    ...
    with st.form("login_form", clear_on_submit=False):
        ...
        if submitted:
            ...
            if ok:
                ...
                # u_or_msg puede ser None o mensaje de error
                st.error("Usuario o contraseña incorrecta.")

def _render_register_small():
    st.markdown("### Registrar usuario (solo admin)")
...
def login_page():
    # page layout: left column login + register button small
    col1, col2 = st.columns([2, 1])
    with col1:
        _render_login_form()
    with col2:
        # Botón que abre un modal-like (simple) para registrar
        if st.button("Registrar usuario"):
            # mostrar el formulario de registro expandido
            _render_register_small()
        else:
            # muestra el formulario de registro compacto
            st.markdown("#### Registro rápido")
            st.write("Pulse 'Registrar usuario' para abrir el formulario de registro.")
